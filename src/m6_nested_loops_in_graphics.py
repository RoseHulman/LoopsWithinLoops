...
def draw_L(window, circle, r, c):
    ...
    for n in range(3):
        for m in range(c):
            bottomcircles = rg.Circle(rg.Point(bx, by), radius)
            bottomcircles.fill_color = circle.fill_color
            bottomcircles.attach_to(window)
            window.render(0.1)
            bx = bx + 2*radius
        by = by + 2*radius
        bx = neworiginalx
...
def draw_wall_on_right(rectangle, n, window):
    ...
    topcorner = rectangle.corner_1
    bottomcorner = rectangle.corner_2
    xtop = topcorner.x
    xbot = bottomcorner.x

    for k in range(n+1):
        for j in range(k):
            newrect = rg.Rectangle(topcorner, bottomcorner)
            newrect.attach_to(window)
            window.render(0.1)
            topcorner.x = topcorner.x + width
            bottomcorner.x = bottomcorner.x + width
        topcorner.x = xtop
        bottomcorner.x = xbot
        topcorner.y = topcorner.y - height
        bottomcorner.y = bottomcorner.y - height
